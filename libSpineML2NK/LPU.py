...
class LPU(Module):
    ...
    def __init__(self, dt, n_dict, s_dict, input_file=None, output_file=None,
                 ...
                 cuda_verbose=False, time_sync=False,visualise=False,components={}):
        ...
        self.sel_out_spk_ids = np.array(self.pm['spike'].ports_to_inds(self.sel_out_spk),
                                        dtype=np.int32)
        if self.visualise:
            self.rt_vis = rt_visualizer.visualizer(self.total_num_gpot_neurons,400,0.1)
    ...
    #################################################################################
            if self.visualise:
                self.rt_vis.update_image(self.V.get())
        if self.total_num_spike_neurons > 0:
    ...
    #TODO
    ## ADAM: Pass in component model, and instantiate neuron with that class.
