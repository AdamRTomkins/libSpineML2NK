...
class SpikingSpineML(BaseAxonHillockModel):
    ...
    def __init__(self, params_dict, access_buffers, dt,
        ...
        # TODO: WIP? IS THIS STATE VARIABLES
        self.internals = OrderedDict([(s.name,0.0) for s in self.SpineMLComponent.ComponentClass.Dynamics.StateVariable])
    ...
    def run_step(self, update_pointers, st=None):
        ...
        print self.update_func.grid
        print self.update_func.block, 
        print st,
        ...
        """
        print "\ninputs:"
        print [self.inputs[k].gpudata for k in self.accesses]
        for k in self.accesses:
                print cuda.from_device(self.inputs[k].gpudata,(1,5),'double')                    

        """
        print "\nparams:"        
        print [self.params_dict[k].gpudata for k in self.params]
        for k in self.params:
            print cuda.from_device(self.params_dict[k].gpudata,(1,5),'double')       
        
        print "\nupdates:"        
        print [update_pointers[k] for k in self.updates]
        for k in self.updates:
            print cuda.from_device(update_pointers[k],(1,5),'double')   
        """
    ...
    def get_update_template(self):
        ...
        print temp_state_variable_assignment


        temp_output_assignment = []

        for a in self.updates:
           temp_output_assignment.append(a +"= g_"+a+"[i]; //outputs\n")

        temp_output_assignment = ''.join(temp_output_assignment)
        print temp_output_assignment
        ...
        print temp_final_output_assignment
        ...
        for a in self.accesses: # Scale this to the right unit!
           ...
           temp_input_assignment.append(a +"= g_"+a+"[i] * %(unitScale)f; // scaled inputs\n" % {'unitScale':unitScale})

        temp_input_assignment = ''.join(temp_input_assignment)
        print temp_input_assignment
               ...
               %(signiture)s)
{
    ...
    int bid = blockIdx.x;
    int nid = bid * num_comps + threadIdx.x;
