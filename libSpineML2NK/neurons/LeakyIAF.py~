    ...
    %(type)s *C)
{
    ...
    if( nid < neu_num ){
        ...
        v = (1-bh)*er + v*bh + r*i*(1.0-bh);  // Added to allow setting the rest potential
...
class LeakyIAF(BaseNeuron):
    def __init__(self, n_dict, spk, dt, debug=False, LPU_id=None,component=None):
        ...
        ## Addition to allow specification of a resting potential, if not supplied, assume zero.
        if 'Er' in n_dict.keys():
            self.Er  = garray.to_gpu( np.asarray( n_dict['Er'], dtype=np.float64 ))
        else: 
            self.Er = garray.to_gpu( np.asarray( np.zeros(self.Vr.shape), dtype=np.float64 ))
    ...
    def update_I(self, synapse_state, st=None):
        self.I.fill(0)
        if self._pre.size>0:
            self._update_I_non_cond.prepared_async_call(self._grid_get_input, # Add here?
        ...
        if self._cond_pre.size>0:
            self._update_I_cond.prepared_async_call(self._grid_get_input,     # add here
                self._block_get_input, st, int(synapse_state),
