...
from neurokernel.LPU.MemoryManager import MemoryManager
...
class LPU(Module):
    ...
    def __init__(self, dt, comp_dict, conn_list, device=0, input_processors=[],
                 ...
                 control_inteface=None, id=None, extra_comps=[],SpineMLComponents={}):
        ...
        self.SpineMLComponents = SpineMLComponents
        if cuda_verbose:
    ...
    def _instantiate_component(self, comp_name):
        ...
        return cls(params_dict, access_buffers, self.dt,
                   ...
                   cuda_verbose=bool(self.compile_options),SpineMLComponents=self.SpineMLComponents)
        
        #class_ins = cls(params_dict, self.dt,
        #           LPU_id=self.LPU_id, debug=self.debug,
        #           cuda_verbose=bool(self.compile_options),SpineMLComponents=self.SpineMLComponents)
       # 
       # self._comps[cls.__name__]['accesses'] = class_ins.accesses ,
        #self._comps[cls.__name__]['updates'] = class_ins.updates,

        #access_buffers = {var:self.memory_manager.get_buffer(var) \
        #                  for var in self._comps[comp_name]['accesses'] \
        #                  if var in self.memory_manager.variables}#

        #class_ins.access_buffers = access_buffers
        #class_ins.set_inputs()
        #return class_ins
        # 1 load component
        # 2 retrieve accesses
        # 3 instantiate component with access

        # OR

        # Load component
        # Instantiate component with no access bufferes
        # Ask for updates and accesses
        # Create Access Bufferes
        # Pass Buffers back to neurons
        # Save buffers back to comps
    ...
    def _load_components(self, extra_comps=[]):
        ...
        # TODO: dont assume first component
        SpineMLComponent = self.SpineMLComponents[self.SpineMLComponents.keys()[0]]        
        updates = []
        accesses = []
        ...
        for cls in comp_classes:
            if not cls.__name__[:4]=='Base':
                self._comps[cls.__name__] = {'accesses': accesses ,
                                     'updates':updates,
                                     'cls':cls} 
