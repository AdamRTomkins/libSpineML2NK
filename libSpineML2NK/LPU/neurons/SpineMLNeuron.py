...
class SpineMLNeuron(BaseNeuron):
    ...
    def __init__(self, n_dict, output, dt, debug=False, LPU_id=None,component=None):
        ...
        for parameter in self.component.Dynamics.StateVariable:
            code = "self.%(name1)s = garray.to_gpu( np.asarray( n_dict['%(name2)s'], dtype=np.float64 ))" % {'name1':parameter.name,'name2':parameter.name}
            exec(code);
            if self.Debug:
                with open('debug_state_variables.py', 'a') as f:
                    f.write(code + '\n')
        ...
        for parameter in self.component.Parameter:
            ...
            exec(code);
            if self.Debug:
                with open('debug_parameters.py', 'a') as f:
                    f.write(code + '\n')
        ...
        if self.debug:
            ...
            for variable in self.component.Dynamics.StateVariable:
                code = "self.%s_file = tables.openFile(self.LPU_id + '_%s.h5', mode='w')" % (variable.name,variable.name)
                exec(code);
                if self.Debug:
                    with open('debug_state_variables.py', 'a') as f:
                        f.write(code + '\n')

                code = "self.%s_file.createEArray('/','array', tables.Float64Atom(), (0,self.num_neurons))" % (variable.name)
                exec(code);
                if self.Debug:
                    with open('debug_state_variables.py', 'a') as f:
                        f.write(code + '\n')
...
""" % { "type": dtype_to_ctype(np.float64),
        ...
        if self.Debug:
            with open('cuda_kernel.txt', 'w') as f:
                f.write(cuda_src)
    ...
    def eval(self, st=None):
        ...
        code = "self.update.prepared_async_call(self.gpu_grid,self.gpu_block,st,self.num_neurons,self.dt,self.output %(state_vars)s, self.I.gpudata %(params)s)" % {"params":p_str,"state_vars":sv_str}
        exec(code);
        if self.Debug:
            with open('debug_input_ports_variables.py', 'w') as f:
                f.write(code + '\n')
        ...
        if self.debug:
            ...
            for variable in self.component.Dynamics.StateVariable:
                code = "self.%s_file.root.array.append(self.%s.get().reshape((1, -1)))" % (variable.name,variable.name)
                exec(code);
                if self.Debug:
                    if self.ran == False :
                        with open('debug_output_variables2.py', 'a') as f:
                            f.write(code + '\n')
            self.ran = True
    ...
    def get_gpu_kernel( self):
        ...
        code = "func.prepare( [ np.int32, np.float64, np.intp %(sv)s, np.intp %(p)s ])" % {"sv":sv_str,"p":p_str}
        exec(code);
        if self.Debug:
            with open('debug_input_prep_variables.py', 'w') as f:
                f.write(code + '\n')
    ...
    def post_run(self):
        ...
        if self.debug:
            self.I_file.close()
            for variable in self.component.Dynamics.StateVariable:
                code = "self.%s_file.close()" % (variable.name)
                exec(code);
                if self.Debug:
                    with open('debug_state_variable_close.py', 'a') as f:
                        f.write(code + '\n')         
